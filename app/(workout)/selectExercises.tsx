...
// ... (MUSCLE_GROUPS and EXERCISES_DB as defined in previous turns)
const MUSCLE_GROUPS = [
...
{ id: 'ba1', name: 'Lat Pulldown', image: 'https://via.placeholder.com/150' },

{ id: 'ba2', name: 'Bent Over Row', image: 'https://via.placeholder.com/150' },

{ id: 'ba3', name: 'Deadlift', image: 'https://via.placeholder.com/150' },

],

'Quads': [

{ id: 'q1', name: 'Barbell Squat', image: 'https://via.placeholder.com/150' },

{ id: 'q2', name: 'Leg Press', image: 'https://via.placeholder.com/150' },

],

// Ensure keys match MUSCLE_GROUPS name property exactly

'Biceps': [{ id: 'b1', name: 'Dumbbell Curl', image: 'https://via.placeholder.com/150' }],

'Triceps': [{ id: 't1', name: 'Triceps Extension', image: 'https://via.placeholder.com/150' }],

'Shoulders': [{ id: 's1', name: 'Military Press', image: 'https://via.placeholder.com/150' }],

'Hamstrings': [{ id: 'h1', name: 'Leg Curl', image: 'https://via.placeholder.com/150' }],

'Abs': [{ id: 'ab1', name: 'Plank', image: 'https://via.placeholder.com/150' }],

'Glutes': [{ id: 'g1', name: 'Hip Thrust', image: 'https://via.placeholder.com/150' }],

'Calves': [{ id: 'ca1', name: 'Calf Raise', image: 'https://via.placeholder.com/150' }],

'Forearms': [{ id: 'f1', name: 'Wrist Curl', image: 'https://via.placeholder.com/150' }],

'Traps': [{ id: 'tr1', name: 'Shrugs', image: 'https://via.placeholder.com/150' }],
...
export default function SelectExercises() {
    ...
    const handleAdd = () => {
        const newlySelected = selectedIds.map(id => {
            const muscleKey = Object.keys(EXERCISES_DB).find(key => EXERCISES_DB[key].find((ex: any) => ex.id === id));
            return EXERCISES_DB[muscleKey!].find((ex: any) => ex.id === id);
        });

        router.push({
            pathname: "/(workout)/addProgramme",
            params: { newExercises: JSON.stringify(newlySelected) }
        });
    };
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.tabContainer}>
                <FlatList data={MUSCLE_GROUPS} horizontal showsHorizontalScrollIndicator={false}
                    renderItem={({ item }) => (
                        <TouchableOpacity style={[styles.muscleTab, selectedMuscle === item.name && styles.activeTab]} onPress={() => setSelectedMuscle(item.name)}>
            ...
            <FlatList data={EXERCISES_DB[selectedMuscle] || []}
                renderItem={({ item }) => (
                    <TouchableOpacity style={styles.exerciseRow} onPress={() => toggleSelection(item.id)}>
                        <Image source={{ uri: item.image }} style={styles.exerciseThumb} />
                        <View style={{ flex: 1 }}><Text style={styles.whiteText}>{item.name}</Text></View>
                        <MaterialIcons name={selectedIds.includes(item.id) ? "check-box" : "check-box-outline-blank"} size={26} color={selectedIds.includes(item.id) ? "#0A84FF" : "#333"} />
                    </TouchableOpacity>
                )}
            />
            ...
            {selectedIds.length > 0 && (
                <View style={styles.buttonContainer}>
                    <TouchableOpacity style={styles.floatingButton} onPress={handleAdd}>
                        <Text style={styles.floatingButtonText}>Add {selectedIds.length} exercises</Text>
                    </TouchableOpacity>
                </View>
            )}
        </SafeAreaView>
...
const styles = StyleSheet.create({
    ...
    activeTab: { opacity: 1 },
    activeIndicator: { height: 2, width: 20, backgroundColor: '#0A84FF', marginTop: 5 },
    ...
    exerciseThumb: { width: 50, height: 50, borderRadius: 8, backgroundColor: '#fff', marginRight: 15 },
    whiteText: { color: 'white', fontWeight: 'bold' },
    buttonContainer: { position: 'absolute', bottom: 30, width: '100%', paddingHorizontal: 20 },
    floatingButton: { backgroundColor: 'white', paddingVertical: 18, borderRadius: 35, alignItems: 'center' },
    floatingButtonText: { color: 'black', fontWeight: 'bold', fontSize: 16 }
});